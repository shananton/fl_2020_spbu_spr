...
Имя состоит из латинских букв, цифр и `_`, где первый символ - не цифра, а все имя не равно `_`.

Пусть `expr` -- это выражение, в котором допустимо использовать
положительные числа (допустимы лидирующие нули, а также запись `0xdeadbeef`),
имена переменных, вызовы функций в виде `f(x, y)`, а также
операторы (украденная табличка кста):
...
Пусть `action` -- это одно из:
- `expr` (вычисление значение выражения, например, вызов функции)
- `var = expr` (присваивание)
- `read var` (считывание числа)
- `write expr` (запись значения выражения)
- `return expr` -- возврат значения из функции

Пусть `statement` -- это одно из:
- `action1, action2, ..., actionN` (например, `x = y` или `x = x + 1, write ans`)
- оператор `if`:
    ```
    if expr then
        statement1
        ...
        statementM
    ```
    где `else` опционально
- сокращенная форма `if` (`else` опционально):
    ```
    if expr then action1, action2, ..., actionN else action1, action2, ..., actionM
    ```
- оператор `while`:
    ```
    while expr do
        ...
        statementN
    ```
- сокращенная форма `while`:
    ```
    while expr do action1, action2, ..., actionN
    ```
- `pass` (необходимо, например, для того, чтобы определить пустое тело функции)

NB: отступы в `if`, `while` и в теле функции ОБЯЗАТЕЛЬНЫ.

Так выглядит определение функции:
    ```
    f(x, y, z) is
        statement1
        statement2
...
Определять две функции с одним именем допустимо лишь тогда, когда у них различное число аргументов.

Программа - это набор определений функций, где ровно одна из них должна иметь имя `main` и не принимать аргументов.
Эта функция выполняется при запуске программы.
Пример программы:
```
f(x) = x ^ 2
main() is
...
5. Затем каждая строка разбивается на токены.
    ...
    # Если в язык добавить, например, оператор '=~', то '/=~' не будет разобрано, как '/' '=~',
    # вместо этого, будет найден '/=', а затем разбор завершится неудачей.
    #
    # Скобки для удобства также включены в число операторов.
    op -> '==' | '/=' | '>=' | '<=' | '&&' | '||' | '<' | '>' | '+' | '-' | '*' | '/' | '^' | '=' | '(' | ')' | ',' | '!'

    kword -> ('if' | 'then' | 'else' | 'while' | 'do' | 'read' | 'write' | 'is' | 'pass' | 'return') [!alnum]
    ...
    ident -> (alpha alnum*) [!alnum] <кроме kword и '_'>
    ```
...
func_def -> func_def_full | func_def_short

func_def_full -> IDENT '(' args_list ')' is NL block 
func_def_short -> IDENT '(' args_list ')' '=' expr NL

args_list = \epsilon | IDENT (',' IDENT)*
...
stmt -> inline_block NL | if_stmt | while_stmt | 'pass' NL

inline_block -> inline_stmt (',' inline_stmt)*
inline_stmt -> inline_assign | inline_read | inline_write | inline_return | expr
inline_assign -> IDENT '=' expr
inline_read -> 'read' IDENT
inline_write -> 'write' expr
inline_return -> 'return' expr
